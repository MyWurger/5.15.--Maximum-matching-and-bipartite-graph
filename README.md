# Программа, находящая максимальные паросочетания. Перед этим определяется, является ли граф двудольным. Результатом является список рёбер (паросочетаний). Для орграфа находятся паросочетания соотнесённого графа.
